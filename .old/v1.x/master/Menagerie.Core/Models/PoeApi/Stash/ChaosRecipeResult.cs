...
namespace Menagerie.Core.Models.PoeApi.Stash
{
    public class ChaosRecipeResult
    {
        ...
        {
            get { return NbBodyArmours < MaxSets; }
        }
        ...
        {
            get { return (int) Math.Floor(NbRings / 2.0f); }
        }

        public bool NeedRings
        {
            get { return NbRingSets < MaxSets; }
        }
        ...
        {
            get { return Nb2HWeapons + Math.Max(Nb1HWeapons / 2, Math.Min(Nb1HWeapons, NbOffHands)); }
        }

        public bool NeedWeapons
        {
            get { return NbWeaponSets < MaxSets; }
        }
        ...
        public int NbSets
        {
            get
            {
                return Math.Min(NbGloves,
                    Math.Min(NbHelmets,
                        Math.Min(NbBoots,
                            Math.Min(NbBelts,
                                Math.Min(NbBodyArmours, Math.Min(NbAmulets, Math.Min(NbRingSets, NbWeaponSets)))))));
            }
        }
